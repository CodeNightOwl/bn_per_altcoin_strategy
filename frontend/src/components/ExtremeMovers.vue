  ...
  <div class="page">
    ...
    <div class="panels">
      <div class="panel">
        <div class="panel-hd gain">
          <div class="hd-left"><span class="hd-dot"></span> 1分急涨 ≥ {{ threshold }}%</div>
          <span class="cnt">{{ data.gainers.length }}</span>
        </div>
        <div class="panel-body">
          <el-table :data="data.gainers" v-loading="loading" style="width:100%" height="100%" @row-click="showDetail" :default-sort="{prop:'change_1m', order:'descending'}">
            <el-table-column prop="symbol" label="币种" width="135">
            ...
            <el-table-column prop="change_1m" label="1分涨跌" width="95" sortable :sort-orders="['descending', 'ascending']" :sort-method="by('change_1m')" align="right">
              <template #default="{ row }">
                <span class="up chg">{{ fmtPct(row.change_1m) }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="volume_24h" label="24H成交量" min-width="135" sortable align="right">
      ...
      <div class="panel">
        <div class="panel-hd loss">
          <div class="hd-left"><span class="hd-dot"></span> 1分急跌 ≤ -{{ threshold }}%</div>
          <span class="cnt">{{ data.losers.length }}</span>
        </div>
        <div class="panel-body">
          <el-table :data="data.losers" v-loading="loading" style="width:100%" height="100%" @row-click="showDetail" :default-sort="{prop:'change_1m', order:'descending'}">
            <el-table-column prop="symbol" label="币种" width="135">
              <template #default="{ row }"><span class="sym">{{ row.symbol }}</span></template>
            </el-table-column>
            <el-table-column prop="price" label="价格" width="120" align="right">
            ...
            <el-table-column prop="change_1m" label="1分涨跌" width="95" sortable :sort-orders="['descending', 'ascending']" :sort-method="by('change_1m')" align="right">
              <template #default="{ row }">
                <span class="down chg">{{ fmtPct(row.change_1m) }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="volume_24h" label="24H成交量" min-width="135" sortable align="right">
              <template #default="{ row }"><span class="col-vol">${{ fmtVol(row.volume_24h) }}</span></template>
    ...
    >
      <div class="detail">
        ...
        <div class="dg">
          <div class="di">
            <span class="dil">1m</span>
            <span :class="clr(selectedCoin?.change_1m)" class="div">{{ fmtPct(selectedCoin?.change_1m) }}</span>
          </div>
          <div class="di">
...
const load = async () => {
  ...
  try {
    ...
    data.value = {
      ...
      losers: [...(res.losers || [])].sort((a, b) => (Math.abs(b.change_1m) || 0) - (Math.abs(a.change_1m) || 0))
    }
...
const initChart = async () => {
  await nextTick()
  if (!chartRef.value) { console.warn('ExtremeMovers: chartRef is null'); return }
  if (!selectedCoin.value) { console.warn('ExtremeMovers: selectedCoin is null'); return }
  try {
    const kd = await coinApi.getCoinKlines(selectedCoin.value.symbol, '1m', 50)
    console.log('ExtremeMovers klines:', kd)
    if (!kd.klines || kd.klines.length < 2) { console.warn('ExtremeMovers: no klines data'); return }
    ...
    console.log('ExtremeMovers: chart created with', kd.klines.length, 'candles')
  } catch (e) { console.error('ExtremeMovers chart:', e) }
}
